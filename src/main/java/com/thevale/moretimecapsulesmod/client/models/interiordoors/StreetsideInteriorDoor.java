package com.thevale.moretimecapsulesmod.client.models.interiordoors;// Made with Blockbench 3.9.2
...
import com.thevale.moretimecapsulesmod.client.renders.exteriors.StreetsideRender;
...
public class StreetsideInteriorDoor extends EntityModel<Entity> implements IInteriorDoorRenderer {
	...
	public void render(DoorEntity door, MatrixStack matrixStack, IVertexBuilder buffer, int packedLight, int packedOverlay) {
		...
		switch (state) {
			case ONE:
				this.RDoor.rotateAngleY = (float) Math.toRadians(EnumDoorTypes.CANON05.getRotationForState(EnumDoorState.ONE));
				this.LDoor.rotateAngleY = (float) Math.toRadians(EnumDoorTypes.CANON05.getRotationForState(EnumDoorState.CLOSED));
				break;
			case BOTH:
				this.LDoor.rotateAngleY = (float) Math.toRadians(EnumDoorTypes.CANON05.getRotationForState(EnumDoorState.BOTH));
				this.RDoor.rotateAngleY = (float) Math.toRadians(EnumDoorTypes.CANON05.getRotationForState(EnumDoorState.ONE));
				break;
			case CLOSED:
				this.RDoor.rotateAngleY = (float) Math.toRadians(EnumDoorTypes.CANON05.getRotationForState(EnumDoorState.CLOSED));
				this.LDoor.rotateAngleY = (float) Math.toRadians(EnumDoorTypes.CANON05.getRotationForState(EnumDoorState.CLOSED));
		}
		matrixStack.push();
